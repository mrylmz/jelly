...
ASTModuleDeclarationRef ASTContextCreateModuleDeclaration(ASTContextRef context, SourceRange location, ASTScopeRef scope,
    ...
    node->base.name              = NULL;
    node->base.type              = NULL;
    node->scope                  = ASTContextCreateScope(context, location, scope, ASTScopeKindGlobal);
...
ASTScopeRef ASTContextCreateScope(ASTContextRef context, SourceRange location, ASTScopeRef parent, ASTScopeKind kind) {
...
void _ASTContextInitBuiltinTypes(ASTContextRef context) {
    ...
    for (Index index = ASTBuiltinTypeKindError + 1; index < AST_BUILTIN_TYPE_KIND_COUNT; index++) {
        ...
        structure->innerScope                = ASTContextCreateScope(context, SourceRangeNull(), globalScope, ASTScopeKindStructure);
